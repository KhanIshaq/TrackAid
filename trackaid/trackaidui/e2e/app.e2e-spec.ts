...
describe('Starting tests for trackaidui', function() {
  ...
  it('website title should be trackaidui', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('trackaidui');
    })
  });

  it('network-name should be trackaid@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trackaid@0.0.1.bna');
    });
  });

  it('navbar-brand should be trackaidui',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trackaidui');
    });
  });
